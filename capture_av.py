...
def get_next_pair_number(save_dir):
    ...
    try:
        files = list(Path(save_dir).glob("*_cal_*.jpg"))
        if not files:
            return 0
        ...
        for file in files:
            try:
                # Extract number from filename like "l_cal_1.jpg"
                base_name = file.stem  # "l_cal_1"
...
def start_client(save_dir, l_port, r_port, l_ip, r_ip):
    ...
    try:
        while not exit_program.is_set():
            ...
            # Prepare filenames
            left_filename = save_path / f"l_cal_{current_pair}.jpg"
            right_filename = save_path / f"r_cal_{current_pair}.jpg"
